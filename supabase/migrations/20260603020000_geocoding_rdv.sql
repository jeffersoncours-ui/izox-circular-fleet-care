-- ===========================================================================
-- B2 : guard race condition dans creer_demande_rdv
-- C2 : creer_demande_rdv accepte p_latitude / p_longitude (DEFAULT NULL)
-- C3 : assigner_rdv propage latitude / longitude vers interventions
-- ===========================================================================

-- ============================================================
-- C2 + B2 — creer_demande_rdv
--   • Nouveaux params p_latitude / p_longitude (DEFAULT NULL, rétrocompat)
--   • Guard : au moins 1 des créneaux proposés doit avoir de la capacité
--     (capacite = COUNT(operators)*2, multi-opérateurs ready)
-- ============================================================

-- Supprimer l'ancienne signature sans lat/lon pour éviter la surcharge ambiguë
DROP FUNCTION IF EXISTS public.creer_demande_rdv(UUID[], JSONB, TEXT, TEXT, TEXT, TEXT);
...
BEGIN
  ...
  -- Guard race condition : au moins 1 créneau proposé doit avoir de la capacité disponible.
  -- Capacité = COUNT(operators) * 2 (scalable multi-opérateurs).
  -- Utilise FOR SHARE implicitement via le COUNT sur interventions.
...
-- ============================================================
-- C3 — assigner_rdv : propager latitude / longitude → interventions
-- ============================================================
