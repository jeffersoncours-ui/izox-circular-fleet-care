...
import {
  Droplets, Wind, Recycle, Printer, Download, Leaf,
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <HeroCard icon={Droplets} label="Eau économisée"  value={totals.water}     unit="L"  color="#2563eb" fill="#dbeafe" />
              <HeroCard icon={Wind}     label="Pollution évitée" value={totals.pollution} unit="L"  color="#059669" fill="#d1fae5" />
              <HeroCard icon={Recycle}  label="Compost"          value={totals.circular}  unit="kg" color="#d97706" fill="#fef3c7" />
              <HeroCard icon={Zap}      label="CO₂ évité"        value={totals.ghg}       unit="kg" color="#7c3aed" fill="#ede9fe" />
            </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {totals.water > 0 && (
                  <div className="flex-1 min-w-[120px] bg-blue-50 border border-blue-200 rounded-xl px-3 py-2 text-center">
                    <p className="text-base font-bold text-blue-700">{Math.round(totals.water / 60)}</p>
                ...
                {totals.ghg > 0 && (
                  <div className="flex-1 min-w-[120px] bg-violet-50 border border-violet-200 rounded-xl px-3 py-2 text-center">
                    <p className="text-base font-bold text-violet-700">{Math.round(totals.ghg / 0.12)}</p>
            ...
            <p className="hidden print:block text-[10px] text-gray-400 mt-8 border-t pt-4">
              ...
              Les valeurs CO₂ sont à recalibrer avec la Base Empreinte ADEME avant toute communication officielle.
              © 2026 IZOX — izox.fr
            </p>
          </>
        )}
...
function HeroCard({
  ...
  return (
    <Card className="p-3 text-center border-border/60 shadow-none" style={{ background: fill }}>
      <Icon className="h-5 w-5 mx-auto mb-1" style={{ color }} />
      <p className="text-lg font-bold leading-tight" style={{ color }}>
        {displayValue}<span className="text-xs font-normal ml-0.5">{unit}</span>
      </p>
  ...
  return (
    <Card className="p-12 text-center border-border/60">
      ...
      <p className="text-sm text-muted-foreground mt-2 max-w-xs mx-auto">
        Vos données d'impact RSE apparaîtront ici après validation des prestations IZOX par votre équipe.
      </p>
    </Card>
  );
}
