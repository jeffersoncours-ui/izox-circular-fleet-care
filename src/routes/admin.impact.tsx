...
import { Leaf, Edit2, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
  ...
  return (
    <>
      <p className="text-xs text-muted-foreground mb-4">
        Les coefficients sont appliqués à chaque clôture d'intervention (multiplicateur = 1 véhicule).
        Modifier une valeur n'affecte PAS les calculs passés (snapshot JSONB conservé).
      </p>
...
function ValidationTab() {
  ...
  const grouped = records.reduce<Record<string, ImpactRecord[]>>((acc, r) => {
    const key = r.intervention_id;
    if (!acc[key]) acc[key] = [];
    acc[key].push(r);
    return acc;
  }, {});

  const handleValidateAll = async (interventionId: string) => {
    if (!user) return;
    setValidating(interventionId);
    try {
      await validateRecordsByIntervention(interventionId, user.id);
      toast.success("Records validés");
      load();
    } catch (e: unknown) {
  ...
  if (Object.keys(grouped).length === 0) {
    return (
      <Card className="p-12 text-center border-border/60">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Tous les enregistrements d'impact ont été validés.
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground">
        {records.length} enregistrement{records.length > 1 ? "s" : ""} en attente
        sur {Object.keys(grouped).length} intervention{Object.keys(grouped).length > 1 ? "s" : ""}.
      </p>
        ...
        return (
          <Card key={interventionId} className="p-5 border-border/60">
            <div className="flex items-start justify-between gap-4">
              ...
              <Button
                size="sm"
                onClick={() => handleValidateAll(interventionId)}
                disabled={validating === interventionId}
                className="shrink-0"
              >
...
function Loader({ className = "" }: { className?: string }) {
  return (
    <div className={`flex justify-center ${className}`}>
      <AlertCircle className="h-5 w-5 animate-pulse text-muted-foreground" />
    </div>
  );
}
