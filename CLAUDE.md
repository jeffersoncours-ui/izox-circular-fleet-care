...
**Illustrations SVG :**
- `src/components/landing/illustrations/HeroCar.tsx` — **vidéo R5 E-Tech** (`public/hero-car-r5.webm` VP9 alpha + `public/hero-car-r5.mp4` H.264 fallback). Affichée DIRECTEMENT sans canvas ni blend CSS. Le fond noir est **détouré en transparence réelle** (canal alpha VP9 `yuva420p`, tag `alpha_mode=1`) baked dans le WebM via `ffmpeg lumakey` — fond transparent natif sur Firefox/Chrome/Edge, zéro JS de rendu, zéro flash noir. Autoplay muet en boucle, fondu d'apparition opacity 0→1 sur `loadeddata`/`canplay` (évite le flash de fond noir au démarrage), iOS unlock au premier toucher, `prefers-reduced-motion` → frame figée à t=1s. **NE PAS revenir au canvas chroma-key** (`drawImage`/`getImageData`) : cassé sur Firefox (vidéo masquée non décodée). **NE PAS utiliser `mix-blend-mode: screen`** : cassé quand le parent `.rv` a un `transform`/`opacity` (crée un stacking context isolé → le noir reste). La couleur de la voiture est figée dans la vidéo (ne suit plus le TweakPanel `--b2c-accent`). ffmpeg : `ffmpeg -i src.webm -vf "lumakey=threshold=0:tolerance=0.10:softness=0.16,format=yuva420p" -c:v libvpx-vp9 -pix_fmt yuva420p -auto-alt-ref 0 -b:v 0 -crf 48 -an hero-car-r5.webm`. Voir `tasks/lessons.md` (session 40)
